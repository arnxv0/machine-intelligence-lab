...
class KNN:
    ...
    def find_distance(self, x):
        ...
        for point in x:
            dp_list = []
            for data_p in self.data:
            	dist = minkowski_distance(point, data_p, self.p)
            	dp_list.append(dist)
            result_matrix.append(dp_list)
    ...
    def k_neighbours(self, x):
        ...
        dist_matrix = []
        idx_matrix = []
        for point_neighs in self.find_distance(x):
        
       	    neighs = point_neighs.copy()
       	    dist_list = []
       	    idx_list = []
       	    for i in range(self.k_neigh):
       	        dist = min(neighs)
       	        neighs.remove(dist)
       	        idx = point_neighs.index(dist)
       	        dist_list.append(dist)
       	        idx_list.append(idx)
       	    
       	    dist_matrix.append(dist_list)
       	    idx_matrix.append(idx_list)
    ...
    def predict(self, x):
        ...
        """
        [dist_matrix, idx_matrix] = self.k_neighbours(x)
        predicted = []
        for point in range(len(idx_matrix)):
            idx_list = idx_matrix[point]
            
            if self.weighted:
                weighted_dict = {}
                for idx in range(len(idx_list)):
                    weighted_dict[self.target[idx_list[idx]]] = 0
                
                for idx in range(len(idx_list)):
                    weighted_dict[self.target[idx_list[idx]]] += (1/dist_matrix[point][idx])
                predicted.append(max(zip(weighted_dict.values(), weighted_dict.keys()))[1])
                
            else:
                pred = []
                for idx in idx_list:
                    pred.append(self.target[idx])
                predicted.append(max(set(pred), key = pred.count))
        return predicted
