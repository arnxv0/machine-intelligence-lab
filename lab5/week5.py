...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        Returns:
            ...
            Ex:
                ...
                Gradient to a and b
        """
        # print("Grad in add", gradients)
        # print("Grad in add", self.grad)
        ...
        print(gradients)
        grad1 = np.dot(gradients, np.ones(gradients.shape[1]))
        grad2 = np.dot(gradients, np.ones(gradients.shape[1]))
        print(gradients)
        print("Grad1 ", grad1)
        print("Grad2 ", grad2)
        return (grad1, grad2)
    ...
    def grad_matmul(self, gradients=None):
        ...
        """
        # print("Grad in mul", gradients)
        # print("Grad in add", self.grad)
        print(gradients.size()[0])
        print(gradients)
        grad1 = np.dot(gradients, self.history[2].arr)
        grad2 = np.dot(gradients, self.history[1].arr)
        print("Grad1 ", grad1)
        print("Grad2 ", grad2)
        return (grad1, grad2)
    ...
    def backward(self, gradients=None):
        ...
        print("hi")
        # if not gradients:
        #     gradients = np.eye(self.grad.shape[1])
        # print("Grad in backward", self.grad)
        if self.history[0] == "leaf":
            ...
            # get the gradient and multiply it
