...
class Tensor:
    ...
    def grad_matmul(self, gradients=None):
        ...
        if gradients is None:
            if self.history[1] == self.history[2] :
                grad1 = 2 * self.history[1].arr
                grad2 = 2 * self.history[1].arr
            else:
                grad1 = np.dot(grad1, np.transpose(self.history[2].arr))
                grad2 = np.dot(np.transpose(self.history[1].arr), grad2)
        else:
            grad1 = np.dot(grad1, np.transpose(self.history[2].arr))
            grad2 = np.dot(np.transpose(self.history[1].arr), grad2)
            grad1 = np.dot(gradients, grad1)
            grad2 = np.dot(gradients, grad2)
        return (grad1, grad2)
